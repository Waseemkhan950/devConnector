...
//@route Get api/profile/me => logged in user's profile
//@desc  Get current user profile
//access Private
router.get("/me", auth, async (req, res) => {
	try {
		const profile = await Profile.findOne({
			user: req.user.id, // finding user's profile with tha certain id
...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			//we save the instance created above; not the model
			await profile.save();
			res.json(profile);
		} catch (error) {
			console.error(error.message);
...
router.get("/", async (req, res) => {
	try {
		const profiles = await Profile.find().populate("user", ["name", "avatar"]); //bringing name and avatar from User doccument and populating it int he profile object
		res.json(profiles);
	} catch (error) {
		console.error(error.message);
		res.status(500).send("Server Error!");
...
router.get("/user/:user_id", async (req, res) => {
	try {
		...
		if (!profile)
			...
			return res.status(400).json({ msg: "there is no profile for this user" });
		// if user found, he profile will be sent back
		res.json(profile);
	} catch (error) {
		console.error(error.message);
		//if the sent id is invalid then this will trigger, making sure only valid id is used.
		if (error.kind == "ObjectId") {
			return res.status(400).json({ msg: "there is no profile for this user" });
		}
		//if error is other than the user id, following will trigger
		res.status(500).send("Server Error!");
	}
});
...
router.put(
	...
	async (req, res) => {  
		...
		if (!errors.isEmpty()) {
            ...
            // destructuring in play
		const { title, company, location, from, to, current, description } =
                  req.body;
            // preparing Experience object to be saved in Profile
		const newExp = {
			title: title,
			company: company,
			location: location,
